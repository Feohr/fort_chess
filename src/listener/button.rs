...
use bevy::{
    ...
    prelude::{
        default, AlignItems, AlignSelf, App, AssetServer, ButtonBundle, Color, Commands, Component,
        Entity, Font, Handle, Image, Interaction, JustifyContent, NodeBundle, Plugin, Query, Res,
        Size, Style, SystemSet, TextBundle, TextStyle, UiColor, UiRect, Val, With,
    },
};
...
/// Type alias for button color query.
type BtnColorQuery<'a> = (&'a Interaction, &'a mut UiColor);
...
impl Plugin for FortButtonPlugin {
    fn build(&self, app: &mut App) {
        app.add_system_set(
            SystemSet::on_enter(FortChessState::StartScreen).with_system(init_btn_obj),
        )
        .add_system_set(
            SystemSet::on_exit(FortChessState::BoardScreen).with_system(despawn_buttons),
        )
...
fn despawn_buttons(mut commands: Commands, buttons: Query<Entity, With<BoardButton>>) {
    commands.despawn_entity(&buttons);
}

/// Creating button object and adding it to resources to be able to call later.
fn init_btn_obj(mut commands: Commands, font: Res<RegFontHandle>, asset_server: Res<AssetServer>) {
    commands.insert_resource(BtnContainer {
        font: font.get().clone(),
        font_size: style::BTN_FONT_SIZE,
        image: asset_server.load("spritesheet/button.png"),
...
fn btn_text_spawn<'a>(commands: &mut ChildBuilder, button: &Res<BtnContainer>, text: &'a str) {
