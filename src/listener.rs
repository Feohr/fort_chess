...
use bevy::{
    prelude::{
        Res, ResMut, EventReader,  Sprite, SpriteBundle, Transform, Vec2, Vec3, Color, Query,
        Entity, With, Commands, Component, CursorMoved, Windows, MouseButton, default,
    },
    input::Input,
};
use fort_builders::{
...
fn click_listener(
    ...
    if !in_pos(m_x, m_y, game.get().len()) { return }
        ...
        match game.get().picked {
            true  => {
                ...
                if let Err(_) = game
                    ...
                    game
                    .get_mut()
                    .update(m_x as i32, m_y as i32, pos)
                    .unwrap();
                    game.get_mut().next();
                    return;
                }
...
pub fn listener(
    ...
    if let Some(window) = windows.get_primary() {
        ...
        click_listener(&mut commands, &mut click, &clicks, &mut game, position.0, position.1);
    }
}
/*-----------------------------------------------------------------------------------------------*/
